...
#users = list(range(helper.params['number_of_total_participants']))
...
if __name__ == '__main__':
    ...
    beta = 0.4
    gamma = 0.6
    bias= torch.ones(helper.params['number_of_total_participants'], device=device)
    rs = torch.zeros(helper.params['number_of_total_participants'], device=device)

    #reputation_dict = { i: 0  for i in range(0, 100)} # number of total participants is 100
    ...
    for epoch in range(helper.start_epoch, helper.params['epochs'] + 1, helper.params['aggr_epoch_interval']):
        ...
        if helper.params['is_random_namelist']:
            if helper.params['is_random_adversary']: 
                ...
                bias = torch.div(bias,bias.sum())
                rs = torch.div(rs,rs,sum())
                #for w in sorted_keys:
                    #sorted_dict[w] = (reputation_dict[w] * f* (1-f)) +f
                key_list = list(sorted_dict.keys())
                rsystem = (beta * C)/2 + (1- beta) *rs + (beta/2) * bias)   revised reputation score
                #agent_name_keys = random.choices(key_list, weights=value_list, k=10) # random choose , maybe don't have advasarial
                #agent_name_keys = random.choices(key_list, weights=value_list, k=10)
                agent_name_keys = random.choices(key_list,weights=rsystem,k=10)
        ...
        if helper.params['aggregation_methods'] == config.AGGR_MEAN:
            ...
            is_updated, names, client_grads = helper.contra_update(helper.target_model, updates)
            #vis_agg_weight(helper,names,weights,epoch,vis,adversarial_name_keys)
            #vis_fg_alpha(helper,names,alphas,epoch,vis,adversarial_name_keys )
            #num_oracle_calls = 1
        ...
        flat_aggre_grad = flatten(alphas)

        phis = torch.zeros(helper.params['number_of_participants'], device=device)
        for i, gradient in enumerate(client_grads):
            phis[i] = F.cosine_similarity(flatten(gradient), flat_aggre_grad, 0, 1e-10) 

        past_phis.append(phis)
        #sum 2D list: result = list(map(sum, array))
        phis_historical = list(map(sum,past_phis[:-1]))


        rs = gamma * phis + (1-gamma) * phis_historical
        
        r#s = torch.div(rs, rs.sum())
        #if helper.params['is_poison']:
